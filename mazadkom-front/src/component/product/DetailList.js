...
class DetailList extends Component {
  ...
  saveBid(bidhistory, liveStockID) {
    ...
    fetch('/api/bidhistory', {
      ...
      dataType: 'json',
      body: JSON.stringify(this.state.bidHistory),
    })
      .then((res) => res.json())
      .then((bidhistory) => this.setState({ bidhistory }));
  }
    ...
    let detailsNodes = this.props.data.map(function (details) {
      ...
      if (Object.keys(self.state.bidHistory).length !== 0) {
        ...
        bidSort = Object.keys(self.bidHistoryObj)
          ...
          .reduce(
            (obj, key) => ({ ...obj, [key]: self.bidHistoryObj[key] }),
            {}
          );
      }
      return (
    ...
    return (
      <div className="detailsList">
        {this.state.bidHistory.length !== 0 && (
          <div className="row">{detailsNodes}</div>
        )}
      </div>
    );
  }
...
class Details extends Component {
  ...
  handleSubmit(event) {
    ...
    let bidHistoryObj = this.props.bidHistory;
            ...
            <div className="col-md-12 ">
              <div>
                <div className="livestock-info">
                  {/* <form action="http://localhost:5000/az">
                    <input type="submit" value="Go to Google" />
                  </form> <button> add room</button> */}
                  <h5>
                    {this.props.breed} - {this.props.id}
                  </h5>
                  <h5>Base Price - ${this.props.basePrice}</h5>
                  {this.props.timeFromServer > 0 && (
                    <BidTimer timeFromServer={this.props.timeFromServer} />
                  )}
                  {this.props.timeFromServer < 0 && <h5>SOLD</h5>}
                </div>
                {this.props.userName !== '' && (
                  <div>
                    {this.state.showBidInput ? (
                      <form
                        className="form-inline bid-form"
                      ...
                      <input
                        type="button"
                        className="btn btn-primary rebid-input"
                        value="ReBid"
                        onClick={this.reBid.bind(this)}
                      />
                    )}
                  </div>
                )}
